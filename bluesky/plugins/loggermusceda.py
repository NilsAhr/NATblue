""" BlueSky loggermusceda — minimal flight-state & conflict logger
...
def init_plugin():
    ...
    global loggermusceda
    loggermusceda = LoggerMusceda()

    config = {
        'plugin_name': 'LOGGERMUSCEDA',
        'plugin_type': 'sim',
    }
...
class LoggerMusceda(Entity):
    ...
    def __init__(self):
        ...
        # Log files
        self.flst = datalog.crelog('FLSTLOG_MUSCEDA', None, flstheader)
        self.conflog = datalog.crelog('CONFLOG_MUSCEDA', None, confheader)
    ...
    # ------------------------------------------------------------------
    @core.timed_function(name='LOGGERMUSCEDA', dt=1.0)
    def update(self, dt):
        """
        ...
        for pair, dur in list(self.duration.items()):
            ...
            if cpf in traf.cd.confpairs_unique:
                ...
                self.conflog.log(
                    ac1, ac2,
                    self.init_alt1[cpf] / ft,
                    self.init_alt2[cpf] / ft,
                    self.dcpa[cpf] / nm,
                    self.tcpa[cpf],
                    self.dist[cpf] / nm,
    ...
    # ------------------------------------------------------------------
    def start_log(self):
        print(f"LOGGERMUSCEDA: FLST and CONF logger started: {self.sim_name}")
        self.flst.start()
        self.conflog.start()
        return True, f'MUSCEDA FLST and CONF logger is ON for simulation: {self.sim_name}.'
