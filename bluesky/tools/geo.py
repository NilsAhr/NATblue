...
# Calculate the closest point along a line segment to a given coordinates
def project_point_on_line(px, py, x1, y1, x2, y2):
    """ Projects a point (px, py) onto the line segment (x1, y1) - (x2, y2) """
    #convert the start (A = (x1, y1)) and end (B = (x2, y2)) of the line into numpy arrays.
    A = np.array([x1, y1]) 
    B = np.array([x2, y2])
    #Convert the aircraft's position (P = (px, py)) into a numpy array.
    P = np.array([px, py])
    
    #Compute the vector from A to B (AB = B - A).
    AB = B - A
    #Compute the vector from A to the aircraft 
    AP = P - A
    
    #projection factor t (determines where the closest point lies)
    #If t = 0: Closest point is A.
    #If t = 1: Closest point is B.
    #If 0 < t < 1: Closest point is on the segment between A and B.
    t = np.dot(AP, AB) / np.dot(AB, AB)
    t = np.clip(t, 0, 1)  # Clamping to the segment
        
    #This gives the exact (latitude, longitude) of the closest point.
    projection = A + t * AB
    #bs.scr.echo(f"GEO: closest point on the line is {projection}")
