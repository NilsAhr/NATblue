...
class ResumeNavigation(Entity, replaceable=True):
    ...
    def select_default(cls):
        ''' Select the default resume navigation method.

            NATBlue default = PastCPANAT (PastCPA + NAT waypoint snap-forward);
            falls back to PastCPA if the NAT variant is unavailable. '''
        default = cls.derived().get('PASTCPANAT') or cls.derived().get('PASTCPA')
        if default:
            default.select()
        return default
